  ...
  try {
    ...
    for (const user of userinfo) {
      ...
      if (accountIndex > 1) {
        ...
        const accountDelay = Math.random() * 60000 + 60000

        printYellow(`账号间随机延时 ${Math.round(accountDelay / 1000)} 秒...`)
        ...
        while (retryCount < maxRetries) {
          ...
          if (ad.status === 1) {
            ...
            if (retryCount < maxRetries) {
              ...
              const retryDelay = Math.random() * 30000 + 30000 // 30~60秒随机延时

              printRed(`第${i}次领取失败，${Math.round(retryDelay / 1000)}秒后重试(第${retryCount}次重试)`)

              await delay(retryDelay)

            } else {

              printRed(`第${i}次领取失败，已达最大重试次数(${maxRetries})`)
